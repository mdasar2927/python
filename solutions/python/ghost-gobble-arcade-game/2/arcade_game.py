...
def win(has_eaten_all_dots, power_pellet_active, touching_ghost):
    ...
    """
    #my notes: (the mistakes i made)
    """
    I first wrote:
    
    return has_eaten_all_dots and power_pellet_active and not touching_ghost
    
    This is wrong because I added extra conditions that the question did not ask for.
    
    The question only says Pac-Man wins if:
    1. He has eaten all the dots.
    2. He has not lost.
    
    Pac-Man loses only when he is touching a ghost and does not have a power pellet.
    
    So, Pac-Man can still win even if he is touching a ghost, as long as the power pellet is active.
    
    My code returns False in that situation, so it is not correct.
    """
